...
<?php
if($_SESSION['nip']){
  $sql = $conn->query("SELECT * FROM guru WHERE nip = '$nip'");
  $row = $sql->fetch_assoc();
...
<form method="post" enctype="multipart/form-data" class="col-md-8" action="?p=editguru">
  ...
  <div class="form-group">
    ...
    <input type="text" readOnly required name="nip" value="<?php echo $row['nip'] ?>" class="form-control" >
  </div>
  <div class="form-group">
    <label for="nama">Nama Guru</label>
    <input type="text" readOnly required name="nama" value="<?php echo $row['nama_guru'] ?>" class="form-control">
  </div>
  <div class="form-group">
...
<div class="form-group">
    ...
    <input type="text" readonly="" required name="pendidikan" class="form-control"  value="<?php echo $row['pendidikan'] ?>">
  </div>
  ...
  <a href="?p=editprofil_guru&id=<?php echo $row['kd_guru'] ?>" name="edit" class="btn btn-primary">Edit Guru</a>
</form>
<?php  } ?>
